...
import (
	...
	"chatgpt-go/pkg/lemur/internal/test/checks"
...
func TestCreateChatCompletionStream(t *testing.T) {
	client, server, teardown := setuplemurTestServer()
	defer teardown()
...
func TestAzureCreateChatCompletionStreamRateLimitError(t *testing.T) {
	wantCode := "429"
	wantMessage := "Requests to the Creates a completion for the chat message Operation under Azure lemur API " +
		"version 2023-03-15-preview have exceeded token rate limit of your current lemur S0 pricing tier. " +
		"Please retry after 20 seconds. " +
		"Please go here: https://aka.ms/oai/quotaincrease if you would like to further increase the default rate limit."

	client, server, teardown := setupAzureTestServer()
	defer teardown()
	server.RegisterHandler("/lemur/deployments/gpt-35-turbo/chat/completions",
